...
public class alphabeta_liandy_eric extends AIModule
{


  private int myAI;
  private int optMove = -1;
  private static final int CUTOFF = 7;  // cutoff > 8 will exceed the allowed 500ms turn time
  ...
  private void buildColLookupTable(){
    ...
    myAI = state.getActivePlayer();
		...
		minimax(state, 0, true, Integer.MIN_VALUE, Integer.MAX_VALUE);
    chosenMove = optMove;
	}
  ...
  private int getPlayerAtCell(final GameStateModule state, int row, int col){
	...
	private int minimax(final GameStateModule state, int depth, boolean playerIsMax, int alpha, int beta)
	{
		if(terminate){
		...
		if(playerIsMax){	// pick max
			...
			for(int i : branchOrdering) {
				if(state.canMakeMove(i)){
					...
					state.makeMove(i);
					val = minimax(state, depth, false, alpha, beta);	// now recurse to the Min player.
          ...
          if(alpha >= beta) {
			...
			for(int i : branchOrdering){
				...
				if(state.canMakeMove(i)){
					state.makeMove(i);
					val = minimax(state, depth, true, alpha, beta);
					// System.out.println(i + "is" + val);
					if(val < beta){
